...
for d, axs in zip(data, axss):
    ...
    # ind_nid = np.random.randint(0, len(root_nid)-1)
        ...
        for i in range(1, num_sample+1, 1):
            ...
            dist.append((smooth_feat - self_feat).cpu())
        ...
        x = n/T 
        y = []
        for i in range(1, smooth_feat.shape[0], 1):
            # y.append(DFT(dist[i] - np.mean(dist[i])))
            # pdb.set_trace()
            y.append(nfft.nfft(neigh_t.cpu(), dist[i] - np.mean(dist[i])))
        # pdb.set_trace()
    ...
    axs[0].set_title('{} Frequency domain Magnitude'.format(d))
    axs[1].stem(x, np.mean(x_phase, 0))
    axs[1].set_title('{} Frequency domain Phase'.format(d))
...
fig.tight_layout()
plt.savefig('feat_dist_50_DFT_all_nfft.png', dpi=300)
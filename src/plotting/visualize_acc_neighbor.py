...
if args.all_in_one:
    path_unifile = f'all_datasets_mean.xlsx'
...
for dataset in datasets:
    ...
    for aggr in aggrs:
        for sampling in samplings:
            for memory in memorys:
                if memory == 'gru' and sampling == 'uni':
                    continue
                means = []
                stds = []
                for scan in scans:
                ...
                if len(means) == len(scans):
                    ...
                    df_all[f"{aggr}+{sampling}+{memory}"] = [(str(round(mean, 4))) for (mean, std) in zip(means, stds)]

...
parser.add_argument('--clip_root_set', action='store_true', help='clip root set to 1M, train:test:val = 70%:15%:15%')
...
is_bipartite = True if args.data in [
    'WIKI', 'REDDIT', 'MOOC', 'mooc', 'LASTFM', 'lastfm', 'Taobao', 'sTaobao', 'MovieLens'] else False

if not os.path.exists('DATA/{}/'.format(args.data)):
...
# idx split
train_edge_end = int(num_edge * 0.7)
val_edge_end = train_edge_end + int(num_edge * 0.15)

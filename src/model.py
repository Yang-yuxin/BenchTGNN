...
class TGNN(torch.nn.Module):
    ...
    def aggregate_messages_and_update_memory(self, blocks, pos_edge_feats):
        ...
        for block, layer in zip(blocks, self.layers):
            ...
            root_node_memory = self.memory.get_memory(block.root_nid) if self.memory is not None else torch.tensor([]).to(self.device)
            neighbor_node_memory = self.memory.get_memory(block.neighbor_nid).reshape(-1, self.memory.dim_memory) \
